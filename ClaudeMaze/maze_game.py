...
class RoomType:
    ...
    def get_color(room_type):
        ...
        colors = {
            RoomType.NORMAL: (0.3, 0.3, 0.3),
            RoomType.START: (0.2, 0.8, 0.2),
            RoomType.END: (0.8, 0.2, 0.2),
            RoomType.SLOW: (0.5, 0.3, 0.1),
            RoomType.TRAP: (0.8, 0.1, 0.1),
            RoomType.SPEED: (0.2, 0.4, 0.8),
            RoomType.SPINNER: (0.8, 0.5, 0.2),
            RoomType.LAUNCHER: (0.6, 0.2, 0.8),
        }
        return colors.get(room_type, (0.3, 0.3, 0.3))
...
def render_floor(world_x, world_z, cell):
    """Render a floor tile"""
    color = RoomType.get_color(cell.room_type)
    
    # Make explored but not currently visible cells dimmer
    if cell.explored:
        brightness = 0.5
    else:
        brightness = 1.0
...
def render_maze(maze, player):
    ...
    player_cell_z = int(player.z / CELL_SIZE)
    ...
    for x in range(maze.width):
        for z in range(maze.height):
            ...
            # Simple fog of war: only render explored cells
            # or cells adjacent to current position
            distance = abs(x - player_cell_x) + abs(z - player_cell_z)
            
            if distance <= 1 or cell.explored:
                # Render floor
                render_floor(world_x, world_z, cell)
