...
class Maze:
    ...
    def remove_entrance_exit_walls(self):
        ...
        # Remove appropriate wall at start (0, 0 - typically at top-left corner)
        # Remove west wall if at left edge
        if sx == 0:
            start_cell.walls['W'] = False
        # Remove north wall if at top edge
        if sz == 0:
            start_cell.walls['N'] = False
        
        # Remove appropriate wall at end (width-1, height-1 - typically at bottom-right corner)
        # Remove east wall if at right edge
        if ex == self.width - 1:
            end_cell.walls['E'] = False
        # Remove south wall if at bottom edge
        if ez == self.height - 1:
            end_cell.walls['S'] = False
